...
class APIService {
  ...
  static Future<DataResponse> fetchLatest() async {
    final response = await http.get(Uri.parse("${dotenv.env['DATABASE_URL']!}/exec?query=SELECT%20ts%20FROM%20phase%20ORDER%20BY%20ts%20ASC%20LIMIT%201%3B&nm=true"));

    return DataResponseFromJson(response.body);
  }

    static Future<DataResponse> fetchOldest() async {
    final response = await http.get(Uri.parse("${dotenv.env['DATABASE_URL']!}/exec?query=SELECT%20ts%20FROM%20phase%20ORDER%20BY%20ts%20DESC%20LIMIT%201%3B&nm=true"));

    return DataResponseFromJson(response.body);
  }

  static Future<DataResponse> fetchPhase(int phase, int pages) async {
    final response = await http.get(Uri.parse("${dotenv.env['DATABASE_URL']!}/exec?query=SELECT%20*%20FROM%20phase%20LATEST%20BY%20ts%20WHERE%20%20phase~'$phase'%20ORDER%20BY%20ts%20DESC%3B&nm=true&limit=${pages * 10}"));
  ...
  static Future<DataResponse> fetchPowerActive(double timeRangeDays) async {
    ...
    final response = await http.get(Uri.parse("${dotenv.env['DATABASE_URL']!}/exec?query=SELECT%20ts%2C%20avg(power_active)%20FROM%0A(SELECT%20ts%2C%20power_active%20FROM%20phase)%20timestamp(ts)%0ASAMPLE%20BY%20${sampleByMinutes.toInt()}m%20ORDER%20BY%20ts%20DESC%20LIMIT%20${limit.toInt()}%3B&nm=true"));
